<?php
// brainfuck.php - Brainfuck Compiler Visualizer (Fixed Scroll)
...
function handleBrainfuckCompile() {
    ...
    if (file_put_contents("{$tmp_dir}/program.bf", $source_code) === false) {
    ...
    $result['stages'][] = [
        'name' => 'Optimization',
        'status' => $optimizationResult['has_errors'] ? 'failed' : 'completed',
    ...
    $result['stages'][] = [
        'name' => 'Execution',
        'status' => $executionResult['has_errors'] ? 'failed' : 'completed',
    ...
    $result['outputs'] = [
        ...
        'execution_trace' => $result['stages'][4]['execution']
    ];
...
// Brainfuck-specific functions
function simulateBrainfuckTokens($code) {
    $tokens = [];
    $errors = [];
    
    // Brainfuck only has 8 valid characters
    $valid_chars = ['>', '<', '+', '-', '.', ',', '[', ']'];
    ...
    foreach ($lines as $lineNum => $line) {
        ...
        foreach ($characters as $charPos => $char) {
            ...
            if (in_array($char, $valid_chars)) {
                ...
                $errors[] = [
                    'type' => 'lexical',
                    'message' => "Invalid Brainfuck character: '{$char}'",
                    'line' => $lineNum + 1,
                    'position' => $charPos + 1,
                    'severity' => 'error'
                ];
    ...
    foreach ($tokens as $token) {
        ...
        if ($token['value'] === '[') {
            ...
            if (!empty($loop_stack)) {
                ...
                if (empty($loop_stack)) {
                    $ast['loops'][] = $current_loop;
                } else {
                    $loop_stack[count($loop_stack)-1]['instructions'][] = $current_loop;
                }
                $loop_depth--;
            }
        } else {
            ...
            if (empty($loop_stack)) {
                $ast['instructions'][] = $instruction;
            } else {
                $loop_stack[count($loop_stack)-1]['instructions'][] = $instruction;
            }
        }
    }
...
function simulateBrainfuckOptimization($code) {
    ...
    if (preg_match('/\+{3,}/', $cleaned)) {
        $optimizations[] = 'Combine multiple increments';
    }
    if (preg_match('/-{3,}/', $cleaned)) {
        $optimizations[] = 'Combine multiple decrements';
    }
    if (preg_match('/>{3,}/', $cleaned)) {
        $optimizations[] = 'Combine multiple pointer moves';
    }
    if (preg_match('/<{3,}/', $cleaned)) {
        $optimizations[] = 'Combine multiple pointer moves';
    }
    ...
    if (strpos($cleaned, '[-]') !== false) {
        $optimizations[] = 'Clear cell optimization';
    }
    
    // Copy/move patterns
    if (strpos($cleaned, '[->+<]') !== false) {
        $optimizations[] = 'Move pattern optimization';
    }
...
function simulateBrainfuckIR($code) {
    ...
    foreach ($tokens as $token) {
        ...
        $instruction = [
            ...
            'op' => $token['value'],
            'desc' => $token['description'] ?? $token['value'],
            'line' => $token['line'],
            'pos' => $token['position']
        ];
...
function simulateBrainfuckExecution($code) {
    ...
    // Initialize tape (30,000 cells as per Brainfuck standard)
    $tape = array_fill(0, 10, 0); // Smaller tape for visualization
    $pointer = 0;
    $output = '';
    $input_buffer = "Hello"; // Simulated input
    ...
    while ($pc < count($bf_instructions) && $step_count < $max_steps) {
        ...
        $step = [
            ...
            'pointer' => $pointer,
            'tape_state' => array_slice($tape, 0, 10),
            'output_so_far' => $output
        ];
        
        switch ($instruction['value']) {
            case '>':
                $pointer = ($pointer + 1) % count($tape);
                break;
            case '<':
                $pointer = ($pointer - 1 + count($tape)) % count($tape);
                break;
            case '+':
            ...
            case '.':
                ...
                $step['output_char'] = chr($tape[$pointer]);
                break;
            case ',':
    ...
    if ($step_count >= $max_steps) {
        $errors[] = [
            'type' => 'execution',
            'message' => 'Execution timeout - possible infinite loop',
            'severity' => 'error'
        ];
    }
...
function generateBrainfuckVisualizationData($result, $stage, $view) {
    ...
    if ($view === 'pipeline') {
        $stages = [
            ...
            ['id' => 'optimization', 'name' => 'Optimization', 'x' => -6, 'y' => 0, 'z' => 0],
            ['id' => 'ir', 'name' => 'IR Generation', 'x' => 6, 'y' => 0, 'z' => 0],
            ['id' => 'execution', 'name' => 'Execution', 'x' => 18, 'y' => 0, 'z' => 0],
        ];
        ...
        if (isset($result['outputs']['ast'])) {
            ...
            $node_id = 0;
            createBrainfuckASTNodes($ast, $nodes, $edges, $node_id, 0, 10, 0, -1);
        }
    } elseif ($view === 'tape') {
        // Create tape visualization
        if (isset($result['outputs']['execution_trace']['tape'])) {
            $tape = $result['outputs']['execution_trace']['tape'];
            $pointer = $result['outputs']['execution_trace']['pointer'] ?? 0;
            
            for ($i = 0; $i < count($tape); $i++) {
                $is_active = $i === $pointer;
                $value = $tape[$i];
                $char = $value >= 32 && $value <= 126 ? chr($value) : '•';
                
                $nodes[] = [
                    'id' => 'cell_' . $i,
                    'name' => "Cell $i: $value ($char)",
                    'type' => 'tape_cell',
                    'color' => $is_active ? '#e74c3c' : '#3498db',
                    'position' => ['x' => $i * 6 - 15, 'y' => 0, 'z' => 0],
                    'size' => 2,
                    'value' => $value,
                    'char' => $char,
                    'is_active' => $is_active,
            ...
            $nodes[] = [
                ...
                'type' => 'pointer',
                'color' => '#e74c3c',
                'position' => ['x' => $pointer * 6 - 15, 'y' => 5, 'z' => 0],
                'size' => 1.5,
                'has_errors' => false,
                'error_count' => 0
            ];
        ...
        $exec_nodes = [
            ['id' => 'start', 'name' => 'Start', 'x' => 0, 'y' => 20, 'z' => 0, 'type' => 'start'],
            ['id' => 'init', 'name' => 'Init Tape', 'x' => 0, 'y' => 10, 'z' => 0, 'type' => 'initialization'],
            ['id' => 'parse', 'name' => 'Parse', 'x' => -10, 'y' => 0, 'z' => 0, 'type' => 'process'],
            ['id' => 'execute', 'name' => 'Execute', 'x' => 0, 'y' => -10, 'z' => 0, 'type' => 'process'],
            ['id' => 'io', 'name' => 'I/O', 'x' => 10, 'y' => 0, 'z' => 0, 'type' => 'io'],
            ['id' => 'end', 'name' => 'End', 'x' => 0, 'y' => -20, 'z' => 0, 'type' => 'end'],
        ];
        
        foreach ($exec_nodes as $node) {
            $nodes[] = [
                'id' => $node['id'],
                'name' => $node['name'],
                'type' => $node['type'],
                'color' => getExecutionNodeColor($node['type']),
        ...
        $exec_edges = [
            ['from' => 'start', 'to' => 'init', 'type' => 'control_flow'],
            ['from' => 'init', 'to' => 'parse', 'type' => 'control_flow'],
            ['from' => 'parse', 'to' => 'execute', 'type' => 'control_flow'],
            ['from' => 'execute', 'to' => 'io', 'type' => 'data_flow'],
            ['from' => 'io', 'to' => 'execute', 'type' => 'data_flow'],
            ['from' => 'execute', 'to' => 'end', 'type' => 'control_flow'],
        ];
        
        foreach ($exec_edges as $edge) {
            $edges[] = $edge;
        }
    }
...
function createBrainfuckASTNodes($node, &$nodes, &$edges, &$node_id, $x, $y, $z, $parent_id) {
    ...
    if ($node_type === 'Program') {
        ...
        $node_name = 'Loop (Depth ' . ($node['depth'] ?? 0) . ')';
    } elseif ($node_type === 'Instruction') {
        $node_name = $node['description'] ?? $node['value'] ?? 'Instruction';
    }
    ...
    $nodes[] = [
        ...
        'name' => $node_name,
        'type' => $node_type,
        'color' => getBrainfuckASTNodeColor($node_type),
    ...
    // Handle instructions
    if (isset($node['instructions']) && is_array($node['instructions'])) {
        foreach ($node['instructions'] as $child) {
            if (is_array($child)) {
                $child_x = $x + 15;
                $child_y = $y - 8 - ($child_index * 8);
                $child_z = $z + ($child_index * 5);
                createBrainfuckASTNodes($child, $nodes, $edges, $node_id, $child_x, $child_y, $child_z, $current_id);
                $child_index++;
            }
        }
    }
    ...
    // Handle loops
    if (isset($node['loops']) && is_array($node['loops'])) {
        foreach ($node['loops'] as $loop) {
            $child_x = $x + 15;
            $child_y = $y - 8 - ($child_index * 8);
            $child_z = $z + ($child_index * 5);
            createBrainfuckASTNodes($loop, $nodes, $edges, $node_id, $child_x, $child_y, $child_z, $current_id);
            $child_index++;
        }
    }
    ...
    <script>
        class BrainfuckCompilerVisualizer3D {
            ...
            async compile() {
                ...
                if (!sourceCode.trim()) {
                    this.showStatus('Please enter Brainfuck source code', 'error');
                    return;
                }
                ...
                try {
                    ...
                    this.showStatus(
                        data.success ? 'Brainfuck compilation successful!' : 'Compilation completed with warnings',
            ...
            updateOutputs(data) {
                ...
                if (data.outputs && data.outputs.tokens) {
                    const tokensText = data.outputs.tokens.map(t => 
                        `Line ${t.line}, Pos ${t.position}: ${t.type.padEnd(12)} = "${t.value}" (${t.description || ''})`
                    ).join('\n');
                ...
                if (data.outputs && data.outputs.ir) {
                    const irText = data.outputs.ir.map((instr, idx) => 
                        `${idx.toString().padStart(3)}: ${instr.op} (${instr.desc}) [Line ${instr.line}, Pos ${instr.pos}]`
                    ).join('\n');
                ...
                if (data.outputs && data.outputs.execution_trace) {
                    ...
                    execText += `Final Output: "${exec.output}"\n`;
                    execText += `Pointer Position: ${exec.pointer}\n`;
                    execText += `Total Steps: ${exec.steps?.length || 0}\n\n`;
                    
                    if (exec.steps && exec.steps.length > 0) {
                        execText += `Step-by-step execution:\n`;
                        exec.steps.slice(0, 20).forEach(step => {
                            execText += `Step ${step.step}: ${step.instruction} (${step.description})\n`;
                            execText += `  Pointer: ${step.pointer}, Tape: [${step.tape_state.join(', ')}]\n`;
                            if (step.output_char) {
                                execText += `  Output: '${step.output_char}'\n`;
                            }
                            execText += '\n';
                        });
                        
                        if (exec.steps.length > 20) {
                            execText += `... and ${exec.steps.length - 20} more steps\n`;
                        }
                    }
                ...
                if (data.errors && data.errors.length > 0) {
                    ...
                    data.errors.forEach((error, index) => {
                        ...
                        errorHtml += `<div class="error-item ${errorClass}">
                            ...
                            ${error.line ? `<div class="error-line" style="font-size: 0.85rem; color: #ffd166;">Line ${error.line}, Position ${error.position}</div>` : ''}
                        </div>`;
                    });
            ...
            createDefaultVisualization() {
                ...
                for (let i = 0; i < 5; i++) {
                    ...
                    sphere.position.set(i * 12 - 24, 0, 0);
            ...
            createNode(node) {
                ...
                if (node.type === 'tape_cell') {
                    geometry = new THREE.BoxGeometry(4, 4, 4);
                } else if (node.type === 'pointer') {
                    geometry = new THREE.ConeGeometry(node.size || 2, 6, 16);
                } else if (node.type === 'start' || node.type === 'end') {
                    geometry = new THREE.CylinderGeometry(node.size || 2.5, node.size || 2.5, 5, 16);
                } else {
                ...
                if (node.type === 'start' || node.type === 'end' || node.is_active) {
                    this.addGlowEffect(mesh);
                }
                
                // Add value display for tape cells
                if (node.type === 'tape_cell' && node.value !== undefined) {
                    this.addValueDisplay(mesh, node.value, node.char);
                }
            }
            ...
            addValueDisplay(mesh, value, char) {
                // Create a small text display above the cell
                const canvas = document.createElement('canvas');
                const context = canvas.getContext('2d');
                canvas.width = 64;
                canvas.height = 32;
                
                context.fillStyle = 'rgba(10, 25, 47, 0.8)';
                context.fillRect(0, 0, canvas.width, canvas.height);
                
                context.font = 'bold 12px Arial';
                context.fillStyle = '#64ffda';
                context.textAlign = 'center';
                context.fillText(value.toString(), canvas.width / 2, 20);
                
                context.font = '10px Arial';
                context.fillStyle = '#8892b0';
                context.fillText(char, canvas.width / 2, 30);
            ...
            addLabels(nodes) {
                ...
                nodes.forEach(node => {
                    // Skip tape cells from labels (they have value displays)
                    ...
                    context.fillStyle = '#64ffda';
                    context.textAlign = 'center';
                    context.fillText(this.truncateText(node.name, 20), canvas.width / 2, 25);
                    
                    // Type
                    context.font = '12px "Inter", sans-serif';
                    context.fillStyle = '#8892b0';
                    context.fillText(`Type: ${node.type}`, canvas.width / 2, 45);
                    
                    // Additional info
                    if (node.value !== undefined) {
                        context.font = '11px "Inter", sans-serif';
                        context.fillStyle = '#00d9a6';
                        context.fillText(`Value: ${node.value}`, canvas.width / 2, 65);
                    }
                    
                    if (node.error_count > 0) {
                        context.fillStyle = '#ff6b6b';
                        context.font = '11px "Inter", sans-serif';
                        context.fillText(`${node.error_count} error(s)`, canvas.width / 2, 85);
                    } else if (node.duration) {
                        context.fillStyle = '#8892b0';
                        context.font = '11px "Inter", sans-serif';
                        context.fillText(`Duration: ${node.duration}ms`, canvas.width / 2, 85);
                    }
            ...
            reset() {
                ...
                // Reset source code to Brainfuck
...
>++.>+.+++++++..+++.>++.<<+++++++++++++++.>.+++.------.--------.>+.>.`;
                ...
                // Reset status
                this.updateProgress(0);
                this.showStatus('Ready to compile Brainfuck code');
            }
            ...
            takeScreenshot() {
                ...
                link.href = dataURL;
                link.download = `brainfuck_compiler_visualization_${Date.now()}.png`;
                link.click();
            }
            ...
            async download(type) {
                ...
                try {
                    ...
                    if (type === 'trace') {
                        a.download = `execution_trace_${this.sessionId}.json`;
                    } else if (type === 'ast') {
                        a.download = `ast_${this.sessionId}.json`;
                    } else {
                        a.download = `program_${this.sessionId}.bf`;
                    }
...
> .`,
                ...
                if (examples[exampleId]) {
                    ...
                    this.showStatus(`Loaded Brainfuck example: ${exampleId}`, 'success');
                }
                ...
                if (intersects.length > 0) {
                    ...
                    if (data) {
                        ...
                        if (data.value !== undefined) {
                            html += `<div style="margin-bottom: 4px; font-size: 0.8rem;"><strong>Value:</strong> ${data.value}</div>`;
                        }
                        
                        if (data.char) {
                            html += `<div style="margin-bottom: 4px; font-size: 0.8rem;"><strong>Char:</strong> '${data.char}'</div>`;
                        }
                        
                        if (data.is_active) {
                            html += `<div style="color: #ff6b6b; margin-top: 6px; font-size: 0.8rem;"><i class="fas fa-bullseye"></i> Active Cell</div>`;
                        }
